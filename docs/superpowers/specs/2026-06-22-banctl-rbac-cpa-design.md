...
**CPA 额度(5h/7天/7天Sonnet)**:fork 新增 `GET /v0/management/account-usage?id=<号>`,用账户自己的 OAuth token 拉 Anthropic `/api/oauth/usage`。Tower `cpaclient.Usage` + `cpa_account_quota` 表 + 发现轮询 → 号库显示 5h/7d/7dS 额度徽章。线上实测端到端:号库返回 `cpaQuota`(5h:0 / 7d:53 / 7dS:8 + 恢复时间)。
