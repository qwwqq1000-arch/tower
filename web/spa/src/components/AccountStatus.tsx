...
export function StatusBadge({ status, limitedUntil, limitReason, pausedUntil, className = '' }: { status?: string; limitedUntil?: number; limitReason?: string; pausedUntil?: number; className?: string }) {
  const limited = status === 'limited';
  const paused = status === 'paused';
  const [now, setNow] = useState(Date.now());
  useEffect(() => {
    const deadline = limited ? limitedUntil : paused ? pausedUntil : 0;
    if (!deadline || deadline <= 0) return;
    const id = setInterval(() => setNow(Date.now()), 1000);
    return () => clearInterval(id);
  }, [limited, limitedUntil, paused, pausedUntil]);
  if (!status) return null;
  const cls = `inline-flex items-center whitespace-nowrap px-1.5 py-0.5 rounded border text-[10px] font-mono ${statusColor(status)} ${className}`;
  if (paused) {
    const cd = pausedUntil && pausedUntil > 0 ? fmtCountdown(pausedUntil - now) : null;
    return <span className={cls}>暂停{cd ? `(${cd})` : ''}</span>;
  }
  if (limited) {
    ...
    return <span className={cls}>{prefix}{cd ? `(${cd})` : '(配额)'}</span>;
  }
...
export function statusRank(status?: string): number {
  switch (status) {
    ...
    case 'half_open':
      return 2;
    case 'paused':
      return 1.5; // session pause — between reserve and cooldown
    case 'reserve':
    case 'affinity':
