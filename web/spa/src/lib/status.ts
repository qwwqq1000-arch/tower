...
export const STATUS_LABELS: Record<string, string> = {
  ...
  affinity: '亲和',  // reserve account actively serving via affinity pin
  banned: '封控·冷却',
  half_open: '半开·探测',
  permanent: '封禁(永久)',
  cooldown: '限流·冷却',
  limited: '限额(配额)', // quota-rotated out of dispatch until quota resets (quota-3)
  paused: '暂停(会话)', // session-sim burst pause — NOT quota exhaustion
  offline: '离线',
  disabled: '禁用',
};
...
export const STATUS_COLORS: Record<string, string> = {
  ...
  reserve: 'bg-muted/15 text-muted border-muted/30', // elastic standby — grey/muted
  affinity: 'bg-green-500/10 text-green-300 border-green-500/30', // reserve with active pin — distinct from 活跃
