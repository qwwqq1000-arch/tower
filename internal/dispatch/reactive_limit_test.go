...
func TestParseLimitReset(t *testing.T) {
	...
	limited, reset := parseLimitReset(500, body, now, kw, nil)
	if !limited {
	...
	if l, _ := parseLimitReset(500, `{"type":"message","content":[]}`, now, kw, nil); l {
	...
	if l, _ := parseLimitReset(500, cpaBody, now, kw, nil); l {
	...
	if _, r := parseLimitReset(500, body, lateNow, kw, nil); r != time.Date(2026, 6, 24, 13, 50, 0, 0, time.UTC).UnixMilli() {
		t.Fatalf("passed-time reset=%s want next-day 13:50", time.UnixMilli(r).UTC())
	}

	// 12am resolves to hour 0.
	if _, r := parseLimitReset(500, "hit your limit resets 12:00am (UTC)", now, kw, nil); time.UnixMilli(r).UTC().Hour() != 0 {
	...
	if l, r := parseLimitReset(500, "you've hit your limit, try again later", now, kw, nil); !l || r != now+5*60*1000 {
	...
	if l, r := parseLimitReset(429, cpaCooling, now, kwCpa, nil); !l || r != now+5*60*1000 {
	...
	if l, _ := parseLimitReset(429, cpaCooling, now, kw, nil); l {
	...
	if l, r := parseLimitReset(500, weekly, now, kw, nil); !l || r != time.Date(2026, 6, 28, 12, 50, 0, 0, time.UTC).UnixMilli() {
	...
	if l, _ := parseLimitReset(200, body, now, kw, []int{429, 500}); l {
		t.Fatal("status not in QuotaLimitStatusCodes must skip the scan")
	}
	if l, _ := parseLimitReset(429, cpaCooling, now, kwCpa, []int{429, 500}); !l {
