		...
		if ok {
			...
			if scfg.SessionSimEnabled {
				...
				if s.Store.BurstShouldPause(winKey, target) {
					...
					s.Store.SetPaused(winKey, scfg.MaxConcurrent, nowMs+pause)
...
func (s *Service) buildCandidates(ctx context.Context, ownerID, model string, cfg policy.Config, longCtx bool) (order []string, allKeys []string, resolver Resolver, keyOwner map[string]string, keyCfg map[string]policy.Config) {
	...
	// Limited (quota-saturated) accounts are excluded: their idle capacity would
	// suppress utilisation and prevent elastic scale-up even when all healthy
	// baseline accounts are saturated.
	...
	for _, sn := range snap {
		if baselineSet[sn.Key] && !sn.Limited {
		...
		if committed {
			...
			if sscfg.SessionSimEnabled && out.Status < 300 {
				...
				if s.Store.BurstShouldPause(key, target) {
					...
					s.Store.SetPaused(key, sscfg.MaxConcurrent, nowMs+pause)
