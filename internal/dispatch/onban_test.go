...
func TestOrchestrator_OnBanFires(t *testing.T) {
	...
	st.Ensure("a", 1)
	var banned []string
	o := &Orchestrator{Store: st, Cfg: state.BreakerCfg{PersistStreak: 1, BaseMs: 1000, MaxMs: 9999, Mult: 2}, MaxAttempts: 5, OnBan: func(k string, status int) { banned = append(banned, k) }}
