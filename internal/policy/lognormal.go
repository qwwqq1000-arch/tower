...
// using seed for a deterministic (per-account) unit-uniform draw.
//
...
//
// The returned value is exp(μ + σ·Φ⁻¹(u)) where u = seedUnit(seed) ∈ (0,1).
...
func SampleLogNormal(p50, p95 float64, seed string) float64 {
	...
	u := seedFrac(seed, "lognormal") // (0,1) deterministic per seed
