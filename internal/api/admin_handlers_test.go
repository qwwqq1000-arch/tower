...
func TestCreateNodeAccountOwner(t *testing.T) {
	...
	// No accountOwnerId supplied → default to the "yanghao" tenant (新号导入默认归属).
	if _, err := q.CreateTenant(ctx, sqlc.CreateTenantParams{
		ID: "u_yanghao_dflt", Username: "yanghao", PwHash: "h", Salt: "s", Role: "tenant", IngestKey: "ik_yanghao_dflt",
	}); err != nil {
		_ = q.SetTenantRole(ctx, sqlc.SetTenantRoleParams{ID: "u_yanghao_dflt", Role: "tenant"})
	}
	yh, err := q.GetTenantByUsername(ctx, "yanghao")
	if err != nil {
		t.Fatalf("seed yanghao tenant: %v", err)
	}
	req2 := httptest.NewRequest(http.MethodPost, "/api/admin/nodes", strings.NewReader(`{"baseUrl":"http://5.6.7.8:8080/","kind":"cpa"}`))
	req2.AddCookie(ck)
	req2.Header.Set("X-Requested-With", "tower")
	rec2 := httptest.NewRecorder()
	router.ServeHTTP(rec2, req2)
	if rec2.Code != http.StatusOK {
		t.Fatalf("create node (default owner) status=%d body=%s", rec2.Code, rec2.Body.String())
	}
	var nodeResp2 struct{ ID string }
	if err := json.NewDecoder(rec2.Body).Decode(&nodeResp2); err != nil {
		t.Fatalf("decode default-owner response: %v", err)
	}
	node2, err := q.GetNode(ctx, nodeResp2.ID)
	if err != nil {
		t.Fatalf("GetNode default: %v", err)
	}
	if node2.AccountOwnerID != yh.ID {
		t.Fatalf("expected default account_owner_id=%s (yanghao), got %q", yh.ID, node2.AccountOwnerID)
	}
}
