...
func NewRouter(pool *pgxpool.Pool, secret string, svc *dispatch.Service, q *sqlc.Queries, secureCookies bool) http.Handler {
	...
	if q != nil {
		mux.HandleFunc("POST /api/admin/nodes", requireAdmin(secret, q, createNodeHandler(q)))
		mux.HandleFunc("GET /api/admin/nodes", requireAdmin(secret, q, listNodesHandler(q)))
		...
		mux.HandleFunc("POST /api/admin/policies/dry-run", requireSuperadmin(secret, q, dryRunPolicyHandler()))
		...
		mux.HandleFunc("GET /api/admin/nodes/{id}/profiles", requireAdmin(secret, q, listProfilesHandler(q)))
		mux.HandleFunc("POST /api/admin/nodes/{id}/accounts/import", requireAdmin(secret, q, importProfileHandler(q)))
		mux.HandleFunc("POST /api/admin/nodes/{id}/oauth/start", requireAdmin(secret, q, oauthStartHandler(q)))
		mux.HandleFunc("POST /api/admin/nodes/{id}/oauth/exchange", requireAdmin(secret, q, oauthExchangeHandler(q)))
		...
		mux.HandleFunc("GET /api/admin/nodes/{id}/features", requireAdmin(secret, q, nodeFeaturesGetHandler(q)))
		mux.HandleFunc("PATCH /api/admin/nodes/{id}/features/{adapter}", requireAdmin(secret, q, nodeFeaturesPatchHandler(q)))
		mux.HandleFunc("POST /api/admin/nodes/{id}/refresh", requireAdmin(secret, q, nodeRefreshHandler(q)))
		mux.HandleFunc("PATCH /api/admin/nodes/{id}/enabled", requireAdmin(secret, q, nodeEnableHandler(q)))
		mux.HandleFunc("GET /api/admin/nodes/{id}/telemetry", requireAdmin(secret, q, nodeTelemetryHandler(q)))
		mux.HandleFunc("GET /api/admin/nodes/{id}/quota", requireAdmin(secret, q, nodeQuotaHandler(q)))
		...
		mux.HandleFunc("POST /api/admin/fallback-channels", requireAdmin(secret, q, createFallbackHandler(q)))
		mux.HandleFunc("PATCH /api/admin/fallback-channels/{id}", requireAdmin(secret, q, updateFallbackHandler(q)))
		...
		mux.HandleFunc("POST /api/me/fallback-channels", requireSession(secret, q, meCreateFallbackHandler(q)))
		mux.HandleFunc("PATCH /api/me/fallback-channels/{id}", requireSession(secret, q, meUpdateFallbackHandler(q)))
