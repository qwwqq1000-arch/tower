...
func NewRouter(pool *pgxpool.Pool, secret string, svc *dispatch.Service, q *sqlc.Queries, secureCookies bool, cipher *crypto.Cipher, pushToken string) http.Handler {
	...
	if q != nil {
		...
		mux.HandleFunc("POST /api/admin/nodes/{id}/update-code", requireAdmin(secret, q, updateNodeCodeHandler(q)))
		...
		mux.HandleFunc("PATCH /api/admin/users/{id}/internal", requireSuperadmin(secret, q, requirePerm(secret, q, loadRolePerms, "users:manage", setUserInternalHandler(q))))
		mux.HandleFunc("GET /api/admin/internal-aging/config", requireSuperadmin(secret, q, getAgingConfigHandler(q)))
		mux.HandleFunc("PATCH /api/admin/internal-aging/config", requireSuperadmin(secret, q, setAgingConfigHandler(q)))
