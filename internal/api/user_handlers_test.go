...
func TestUserMgmtAndChangePassword(t *testing.T) {
	...
	ck := adminCookie(t, secret) // sub=u_admin role=admin
	...
	// change own password (session sub = tenant1's id)
	tok := auth.IssueSession(secret, u.ID, "admin", nowUnix(), 3600)
	...
	// non-admin (viewer) cannot list users → 403
	vtok := auth.IssueSession(secret, "u_v", "viewer", nowUnix(), 3600)
	if rec := do("GET", "/api/admin/users", "", &http.Cookie{Name: "tower_session", Value: vtok}); rec.Code != 403 {
