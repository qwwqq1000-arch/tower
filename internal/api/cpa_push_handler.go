...
// the tenant (username, default "yanghao"). Duplicate node IP → the existing node is
		...
		if kind == "cpa" {
			...
			for _, p := range merProfiles {
				...
				if _, aerr := q.AssignAccount(r.Context(), sqlc.AssignAccountParams{
					NodeID: nodeID, AccountID: accID, ProfileID: p.ID, Egress: "", Weight: 100, Role: "baseline", SlotID: "", BoundAt: now,
				}); aerr != nil {
					continue
				}
				registered++
