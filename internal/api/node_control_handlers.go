...
func nodeRefreshHandler(q *sqlc.Queries, cipher *crypto.Cipher) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Re-probe /health and refresh the cache so the node list shows the new
		// login state right after the OAuth wizard closes, not on the next 30s tick.
		if h, herr := cl.Health(r.Context()); herr == nil {
			setHealthCache(n.ID, cachedHealth{
